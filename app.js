...
const conditionLegend=panel.querySelector('#fixedAssetConditionLegend');if(conditionLegend)conditionLegend.innerHTML=conditionCounts.map(item=>'<div><span><i style="background:'+(item[0]==='Damage'?'#d12a31':'#16866a')+'"></i>'+item[0]+'</span><b><strong>'+item[1].toLocaleString()+'</strong><small> asset'+(item[1]===1?'':'s')+'</small></b></div>').join('');
  ...
  window.renderFixedAssetsKpis=(metrics=source,scopeLabel=metrics.scopeLabel||'Selected asset records')=>{
    ...
    const assetTabs=[...assetTabCard.querySelectorAll('[data-fixed-asset-tab]')],escapeAssetCell=value=>String(value??'—').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char])),formatAssetValue=value=>Number(value||0).toLocaleString('en-US');
    const renderAssetTab=view=>{assetTabs.forEach(tab=>{const active=tab.dataset.fixedAssetTab===view;tab.classList.toggle('active',active);tab.setAttribute('aria-selected',String(active))});assetTabCard.querySelector('.fixed-asset-company-table-card')?.remove();if(view!=='company')return;const companies=Object.entries(assetRecords.reduce((groups,record)=>{const company=record.c||'Unspecified';if(!groups[company])groups[company]={count:0,amount:0,good:0,damage:0};groups[company].count+=1;groups[company].amount+=Number(record.a)||0;if(record.o==='Damage')groups[company].damage+=1;else if(record.o==='Good')groups[company].good+=1;return groups},{})).sort((left,right)=>right[1].count-left[1].count),companyTable=document.createElement('section');companyTable.className='unified-table-card fixed-asset-company-table-card';companyTable.innerHTML='<div class="unified-table-head"><div class="unified-table-title"><div><h2>Company Asset Inventory</h2><p>Asset condition, inventory, and purchase value by company</p></div></div></div><div class="unified-table-scroll"><table class="unified-data-table"><thead><tr><th>Company</th><th>Good Assets</th><th>Damaged Assets</th><th>Total Assets</th><th>Purchased (MMK)</th></tr></thead><tbody>'+companies.map(([company,summary])=>'<tr><td>'+escapeAssetCell(company)+'</td><td>'+formatAssetValue(summary.good)+'</td><td class="fixed-asset-damage-count">'+formatAssetValue(summary.damage)+'</td><td>'+formatAssetValue(summary.count)+'</td><td>'+formatAssetValue(summary.amount)+'</td></tr>').join('')+'</tbody></table></div>';companyTable.querySelector('.unified-table-title')?.insertAdjacentHTML('afterbegin','<span class="unified-table-title-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 21h16M6 21V7l6-4v18M12 10h6v11M9 9h1M9 13h1M9 17h1M15 14h1M15 18h1"/></svg></span>');assetTabCard.append(companyTable)};
    const sortCompanyRows=()=>{const body=assetTabCard.querySelector('.fixed-asset-company-table-card tbody');if(!body)return;[...body.rows].sort((left,right)=>Number(right.cells[3].textContent.replace(/[^\d.-]/g,''))-Number(left.cells[3].textContent.replace(/[^\d.-]/g,''))).forEach(row=>body.append(row));const table=body.closest('table'),card=body.closest('.fixed-asset-company-table-card'),count=body.rows.length,totalAssets=[...body.rows].reduce((sum,row)=>sum+Number(row.cells[3].textContent.replace(/[^\d.-]/g,'')),0),totalAmount=[...body.rows].reduce((sum,row)=>sum+Number(row.cells[4].textContent.replace(/[^\d.-]/g,'')),0),goodCount=assetRecords.filter(record=>record.o==='Good').length,damageCount=assetRecords.filter(record=>record.o==='Damage').length,foot=table.tFoot||table.createTFoot();foot.innerHTML='<tr><th>GRAND TOTAL</th><th>'+formatAssetValue(goodCount)+'</th><th>'+formatAssetValue(damageCount)+'</th><th>'+formatAssetValue(totalAssets)+'</th><th>'+formatAssetValue(totalAmount)+'</th></tr>';let footer=card.querySelector('.unified-table-footer');if(!footer){footer=document.createElement('footer');footer.className='unified-table-footer';card.append(footer)}footer.textContent='Showing '+count+' of '+count+' companies'};
    ...
    panel.querySelector('#fixedAssetResetFilters')?.addEventListener('click',()=>requestAnimationFrame(refreshCompanyFixedAssets));
