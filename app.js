...
/* Keep a dedicated copy for drill-down rows after the dashboard initializes. */
fetch('data/budget-expense-data.js?v=20260914-budget-all-sheets').then(response=>response.text()).then(text=>{
  const match=text.match(/^window\.budgetExpenseData=(.*);\s*$/s);
  if(match)window.__budgetExpenseData=JSON.parse(match[1]);
}).catch(()=>{});
(()=>{
  const money=value=>new Intl.NumberFormat('en-US',{maximumFractionDigits:0}).format(Number(value)||0);
  const periodFromRow=row=>{const match=row.cells[0]?.textContent.match(/(?:Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|Jan|Feb|Mar)-\d{2}/);return match?.[0]||''};
  const companyFromRow=row=>{let current=row.previousElementSibling;while(current){if(current.matches('.budget-company-row[data-budget-company]'))return current.dataset.budgetCompany;current=current.previousElementSibling}return''};
  const removeDetails=table=>table.querySelectorAll('.budget-month-category-row').forEach(row=>row.remove());
  window.addEventListener('click',event=>{
    const periodRow=event.target.closest('#budgetPortfolioBody .budget-monthly-row');
    if(!periodRow)return;
    const table=periodRow.closest('table'),company=table?.querySelector('.budget-company-row.selected')?.dataset.budgetCompany||companyFromRow(periodRow),month=periodFromRow(periodRow),source=document.getElementById('budgetExpenseDashboard')?.__budgetExpenseDetailData;
    if(!table||!company||!month||!source)return;
    const alreadyOpen=periodRow.classList.contains('category-detail-open');
    removeDetails(table);
    table.querySelectorAll('.budget-monthly-row.category-detail-open').forEach(row=>row.classList.remove('category-detail-open'));
    if(alreadyOpen)return;
    const budget=(source.budgets||[]).filter(row=>row.company===company&&row.month===month);
    const actual=(source.expenses||[]).filter(row=>row.company===company&&row.month===month);
    const categories=[...new Set([...budget,...actual].map(row=>row.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
    const rows=categories.map(category=>{
      const approved=budget.filter(row=>row.category===category).reduce((sum,row)=>sum+(Number(row.amount)||0),0);
      const spent=actual.filter(row=>row.category===category).reduce((sum,row)=>sum+(Number(row.amount)||0),0);
      const variance=approved-spent,utilization=approved?spent/approved*100:0,status=utilization<=80?'On Budget':(utilization<=100?'Review Budget':'Over Budget');
      return `<tr class="budget-month-category-row"><td>${category}</td><td>${money(approved)}</td><td>${money(spent)}</td><td class="${variance<0?'budget-overrun':''}">${money(variance)}</td><td><div class="budget-utilization"><span><i style="width:${Math.min(100,utilization)}%"></i></span><b>${utilization.toFixed(1)}%</b><em class="budget-status ${utilization>100?'over':(utilization>80?'review':'on')}">${status}</em></div></td></tr>`;
    });
    const fragment=document.createDocumentFragment();
    rows.forEach(markup=>{const template=document.createElement('template');template.innerHTML=markup.trim();fragment.append(template.content.firstElementChild)});
    periodRow.parentNode.insertBefore(fragment,periodRow.nextSibling);
...
(()=>{
  ...
  const bind=()=>document.querySelectorAll('#budgetPortfolioBody .budget-monthly-row:not([data-category-expand-bound])').forEach(periodRow=>{
    ...
    periodRow.addEventListener('click',event=>{
      ...
      if(wasOpen)return;
      const approvedRows=(source.budgets||[]).filter(item=>item.company===company&&item.month===month),actualRows=(source.expenses||[]).filter(item=>item.company===company&&item.month===month),categories=[...new Set([...approvedRows,...actualRows].map(item=>item.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
...
(function(){
  ...
  if(panel&&source)panel.addEventListener('click',event=>{
    ...
    const selectedCategory=document.getElementById('budgetCategoryFilter')?.value||'all',matchesCategory=row=>selectedCategory==='all'||row.category===selectedCategory,budgetRows=source.budgets.filter(row=>row.company===company&&row.month===month&&matchesCategory(row)),expenseRows=source.expenses.filter(row=>row.company===company&&row.month===month&&matchesCategory(row)),categories=[...new Set([...budgetRows,...expenseRows].map(row=>row.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
...
(()=>{
  const update=()=>{
    ...
    const tab=[...card.querySelectorAll('.manpower-planning-tab')].find(button=>button.textContent.includes('Site Coverage'));
    if(tab&&tab.lastChild?.nodeValue!=='Company Coverage')tab.lastChild.nodeValue='Company Coverage';
