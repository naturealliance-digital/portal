...
(function(){
 function renderCompanyRanking(){
  ...
  chart.parentElement.style.height='350px';const selectedType=type.value==='line'?'line':'bar',isLine=selectedType==='line';type.disabled=false;const dark=document.body.classList.contains('dark'),text=dark?'#e8d1cc':'#806864',grid=dark?'#553137':'#eaded9',card=dark?'#32171e':'#fffaf7',ctx=chart.getContext('2d'),gradient=ctx.createLinearGradient(0,0,chart.clientWidth||700,0);
...
(function(){
 ...
 function ensurePanel(){
  ...
  panel=document.createElement('section');panel.id='serviceTicketsDashboard';panel.className='service-ticket-dashboard';panel.hidden=true;
 ...
 function renderShell(){
  ...
  panel.innerHTML='<section class="ticket-kpis"><article class="ticket-kpi tone-total"><span class="ticket-kpi-icon">🎫</span><div><b>Total Tickets</b><small>Selected period</small></div><strong id="ticketTotal">0</strong></article><article class="ticket-kpi tone-company"><span class="ticket-kpi-icon">▦</span><div><b>Companies</b><small>With ticket activity</small></div><strong id="ticketCompanies">0</strong></article><article class="ticket-kpi tone-problem"><span class="ticket-kpi-icon">⚠</span><div><b>Problem Types</b><small>Distinct categories</small></div><strong id="ticketProblems">0</strong></article><article class="ticket-kpi tone-duration"><span class="ticket-kpi-icon">◷</span><div><b>Average Duration</b><small>Resolution time</small></div><strong id="ticketDuration">0m</strong></article></section><section class="ticket-filter-card"><div class="ticket-filter-heading"><div><h2>Ticket Analytics</h2><p>Explore company demand and recurring issues over any period</p></div><button type="button" id="ticketReset" class="btn">Reset filters</button></div><div class="ticket-filter-grid"><label><span>Period</span><select id="ticketRange" class="filter"><option value="all">All data</option><option value="month">Monthly</option><option value="3m">Last 3 months</option><option value="6m">Last 6 months</option><option value="year">Yearly</option><option value="custom">Custom range</option></select></label><label id="ticketMonthField"><span>Month</span><select id="ticketMonth" class="filter">'+monthOptionList()+'</select></label><label id="ticketYearField"><span>Year</span><select id="ticketYear" class="filter">'+optionList(years,state.year)+'</select></label><label id="ticketFromField"><span>From</span><input id="ticketFrom" class="filter" type="date"></label><label id="ticketToField"><span>To</span><input id="ticketTo" class="filter" type="date"></label><label><span>Company</span><select id="ticketCompany" class="filter"><option>All companies</option>'+optionList(companies,state.company)+'</select></label><label><span>Problem</span><select id="ticketProblem" class="filter"><option>All problems</option>'+optionList(problems,state.problem)+'</select></label></div></section><section class="ticket-chart-grid"><article class="card ticket-chart-card"><div class="ticket-chart-title"><div><h2>Tickets by Company</h2><p>Support demand ranked from highest to lowest</p></div><span id="ticketCompanyChartTotal">0 tickets</span></div><div class="ticket-bar-canvas"><canvas id="ticketCompanyChart"></canvas></div></article><article class="card ticket-chart-card ticket-pie-card"><div class="ticket-chart-title"><div><h2>Problems by Category</h2><p>Issue distribution for the selected period</p></div><span id="ticketProblemChartTotal">0 types</span></div><div class="ticket-pie-layout"><div class="ticket-pie-canvas"><canvas id="ticketProblemChart"></canvas></div><div id="ticketProblemLegend" class="ticket-problem-legend"></div></div></article></section><section class="card ticket-table-card"><div class="ticket-table-head"><div><h2>Company Assignment Summary</h2><p>Ticket volume by company and assigned team member</p></div><input id="ticketSearch" class="search" placeholder="Search companies..."></div><div class="ticket-table-scroll"><table class="ticket-data-table ticket-assignment-matrix"><thead><tr><th>Company</th><th>Total</th>'+assignees.map(name=>'<th>'+clean(name)+'</th>').join('')+'</tr></thead><tbody id="ticketTableBody"></tbody><tfoot id="ticketTableTotal"></tfoot></table></div><div class="ticket-table-foot" id="ticketTableFoot"></div></section>';
  const assigneeField=document.createElement('label');assigneeField.innerHTML='<span>Assigned To</span><select id="ticketAssignee" class="filter"><option value="All assignees">All assignees</option>'+assignees.map(name=>'<option value="'+clean(name)+'">'+clean(assigneeLabel(name))+'</option>').join('')+'</select>';panel.querySelector('.ticket-filter-grid').append(assigneeField);
  const tableTitle=panel.querySelector('.ticket-table-head>div');if(tableTitle){tableTitle.classList.add('ticket-table-title');tableTitle.querySelector('h2').textContent='Service Ticket Allocation';tableTitle.querySelector('p').textContent='Tickets assigned to team members by company';tableTitle.insertAdjacentHTML('afterbegin','<span class="ticket-table-title-icon" aria-hidden="true">▦</span>')}
  const companyChartTitle=panel.querySelector('#ticketCompanyChartTotal')?.closest('.ticket-chart-title');if(companyChartTitle){companyChartTitle.querySelector('h2').textContent='Service Ticket Distribution';companyChartTitle.querySelector('p').textContent='Ticket volume across supported companies';companyChartTitle.querySelector('#ticketCompanyChartTotal').outerHTML='<select id="ticketCompanyChartType" class="filter ticket-chart-type" aria-label="Company chart type"><option value="bar">Bar chart</option><option value="line">Line chart</option></select>';panel.querySelector('#ticketCompanyChartType').value=state.companyChartType;panel.querySelector('#ticketCompanyChartType').addEventListener('change',event=>{state.companyChartType=event.target.value;drawCharts(filteredRows())})}
  const problemCard=panel.querySelector('.ticket-pie-card'),problemTitle=problemCard?.querySelector('.ticket-chart-title');if(problemTitle){problemTitle.querySelector('h2').textContent='Service Error Distribution';problemTitle.querySelector('p').textContent='Recorded tickets across nine error categories';problemTitle.querySelector('#ticketProblemChartTotal')?.remove();problemTitle.insertAdjacentHTML('beforeend','<select id="ticketPieProblemFilter" class="filter ticket-pie-filter" aria-label="Filter issue distribution by problem"><option value="All problems">All problems</option>'+optionList(problems,state.problem)+'</select>');panel.querySelector('#ticketPieProblemFilter').value=state.problem;panel.querySelector('#ticketPieProblemFilter').addEventListener('change',event=>{state.problem=event.target.value;panel.querySelector('#ticketProblem').value=state.problem;applyFilters()})}
  panel.querySelectorAll('.ticket-assignment-matrix thead th').forEach((heading,index)=>{if(index>=2)heading.textContent=assigneeLabel(assignees[index-2])});
  ...
  panel.querySelectorAll('#ticketFrom,#ticketTo').forEach(input=>{input.classList.add('ticket-date-input');input.addEventListener('click',()=>{if(typeof input.showPicker==='function')try{input.showPicker()}catch(e){}})});
 ...
 function drawCharts(rows){
  const dark=document.body.classList.contains('dark'),text=dark?'#d9c4c2':'#806864',grid=dark?'#553137':'#eaded9',companyCounts=countBy(rows,'company'),problemCounts=countBy(rows,'problem'),problemColors=['#d12a31','#f06428','#d6a13b','#7656b5','#3194ad','#16866a','#b94f78','#5677b9','#9a6a3a'];
  ...
  const companyCanvas=document.getElementById('ticketCompanyChart'),companyContext=companyCanvas.getContext('2d'),companyGradient=companyContext.createLinearGradient(0,0,companyCanvas.clientWidth||700,0);companyGradient.addColorStop(0,dark?'#c94a42':'#d12a31');companyGradient.addColorStop(1,dark?'#ef8563':'#f38c47');
  ticketBarChart=new Chart(companyCanvas,{type:isLine?'line':'bar',data:{labels:companyCounts.map(item=>item[0]),datasets:[{data:companyCounts.map(item=>item[1]),backgroundColor:isLine?(dark?'rgba(255,135,85,.18)':'rgba(224,82,56,.14)'):companyGradient,borderColor:dark?'#ff8755':'#d12a31',borderWidth:isLine?2.5:1.5,borderRadius:isLine?0:7,barThickness:isLine?undefined:18,pointRadius:isLine?4:0,pointHoverRadius:isLine?6:0,pointBackgroundColor:dark?'#ff9a70':'#d12a31',fill:isLine,tension:isLine?.3:0}]},options:{indexAxis:isLine?'x':'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{displayColors:false}},scales:{x:{beginAtZero:!isLine,grid:{color:grid},ticks:{color:text,precision:isLine?undefined:0,font:{family:'Poppins',size:9},maxRotation:isLine?35:0,minRotation:0}},y:{beginAtZero:isLine,grid:{display:isLine,color:grid},ticks:{color:text,precision:isLine?0:undefined,font:{family:'Poppins',size:9}}}}}});
 ...
 function applyFilters(){
  ...
  updateControlVisibility();panel.querySelector('#ticketTotal').textContent=rows.length.toLocaleString();panel.querySelector('#ticketCompanies').textContent=companyCount;panel.querySelector('#ticketProblems').textContent=problemCount;panel.querySelector('#ticketDuration').textContent=formatDuration(average);
  ...
  panel.querySelector('#ticketTableBody').innerHTML=summary.map(([company,item])=>'<tr><td><b>'+clean(company==='Pyay'?'PYAY':company)+'</b></td><td><strong>'+item.total+'</strong></td>'+assignees.map(name=>'<td class="ticket-count-edit" contenteditable="true" inputmode="numeric" role="spinbutton" aria-label="'+clean(company)+' tickets assigned to '+clean(assigneeLabel(name))+'" data-company="'+clean(company)+'" data-assignee="'+clean(name)+'">'+(item.people[name]||'')+'</td>').join('')+'</tr>').join('');
  panel.querySelectorAll('.ticket-count-edit').forEach(cell=>{cell.dataset.original=cell.textContent.trim()||'0';cell.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();cell.blur()}if(!/^[0-9]$/.test(event.key)&&!['Backspace','Delete','ArrowLeft','ArrowRight','Tab','Home','End'].includes(event.key))event.preventDefault()});cell.addEventListener('blur',()=>updateAssignment(cell.dataset.company,cell.dataset.assignee,cell.textContent.trim(),rows))});
