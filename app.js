...
(()=>{
  const update=()=>{
    const tab=[...document.querySelectorAll('#manpowerPlanningCard .manpower-planning-tab.active')].find(button=>button.textContent.includes('Workforce Plan'));
  ...
  Chart.register({
    ...
    beforeUpdate(chart){
      ...
      if(colorPages){
        ...
        colorPages.backgroundColor=isBar?(dark?'#4eb4cd':'#16866a'):'transparent';
        colorPages.hoverBackgroundColor=colorPages.backgroundColor;
      ...
      if(bwPages){
        ...
        bwPages.backgroundColor=isBar?(dark?'#FF8755':'#D12A31'):'transparent';
        bwPages.hoverBackgroundColor=bwPages.backgroundColor;
    ...
    beforeDatasetsUpdate(chart){
      if(chart.canvas?.id!=='copierMonthlyChart'||!chart.chartArea)return;
      const dark=document.body.classList.contains('dark'),isBar=chart.config.type==='bar',area=chart.chartArea,ctx=chart.ctx;
      const barGradient=(start,end)=>context=>{const bar=context.chart.getDatasetMeta(context.datasetIndex).data[context.dataIndex],width=Number(bar?.width)||Math.max(20,area.width/16),center=Number(bar?.x)||area.left+width/2,fill=ctx.createLinearGradient(center-width/2,0,center+width/2,0);fill.addColorStop(0,start);fill.addColorStop(1,end);return fill};
      const [colorPages,bwPages]=chart.data.datasets;
      if(colorPages){
        colorPages.backgroundColor=isBar?barGradient(dark?'#229681':'#16866a',dark?'#72d5e5':'#4eb4cd'):'transparent';
        colorPages.hoverBackgroundColor=colorPages.backgroundColor;
        colorPages.fill=false;
      }
      if(bwPages){
        bwPages.backgroundColor=isBar?barGradient(dark?'#e95148':'#d12a31',dark?'#ff9569':'#ff8755'):'transparent';
        bwPages.hoverBackgroundColor=bwPages.backgroundColor;
        bwPages.fill=false;
      }
    }
...
(()=>{
  const update=()=>{
    ...
    const tab=card.querySelector('.manpower-planning-tab[data-tab="onsite"]');
...
(()=>{
  ...
  const icon={
    ...
    assetTypes:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="4" y="4" width="12" height="12" rx="2"/><path d="M8 8h4M8 12h4M8 16h4M8 20h10a2 2 0 0 0 2-2V8"/></svg>',
  ...
  let companyChart=null,conditionChart=null,themeObserver=null,themeRefresh=false,companyChartType='bar';
  const setupFilters=()=>{
    ...
    const renderCharts=selected=>{
      ...
      if(companyChart)companyChart.destroy();if(conditionChart)conditionChart.destroy();
      if(companyCanvas){
...
const conditionLegend=panel.querySelector('#fixedAssetConditionLegend');if(conditionLegend)conditionLegend.innerHTML=conditionCounts.map(item=>'<div><span><i style="background:'+(item[0]==='Damage'?'#d12a31':'#16866a')+'"></i>'+item[0]+'</span><b><strong>'+item[1].toLocaleString()+'</strong><small> asset'+(item[1]===1?'':'s')+'</small></b></div>').join('');themeRefresh=false;
  ...
  window.renderFixedAssetsKpis=(metrics=source,scopeLabel=metrics.scopeLabel||'Selected asset records')=>{
    if(companyChart)companyChart.destroy();if(conditionChart)conditionChart.destroy();if(themeObserver)themeObserver.disconnect();companyChart=null;conditionChart=null;themeObserver=null;
    const cards=[
      ['tone-orange',icon.assets,'Total Assets','Recorded asset inventory',format(metrics.totalAssets)],
      ['tone-green',icon.assetTypes,'Total Asset Types','Asset types found',format(new Set((assetsData.records||[]).map(record=>record.t).filter(Boolean)).size)],
