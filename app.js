...
function nav(){queueMicrotask(()=>document.querySelectorAll('#manpowerPlanningCard .manpower-planning-tab').forEach(tab=>tab.classList.remove('manpower-planning-tab')));return '<nav class="unified-tabs">'+Object.entries(tabs).map(([k,v])=>'<button class="unified-tab manpower-planning-tab '+(k===selected?'active':'')+'" data-tab="'+k+'">'+icons[k]+'<span>'+v+'</span></button>').join('')+'</nav>'}
...
const conditionLegend=panel.querySelector('#fixedAssetConditionLegend');if(conditionLegend)conditionLegend.innerHTML=conditionCounts.map(item=>'<div><span><i style="background:'+(item[0]==='Damage'?'#d12a31':'#16866a')+'"></i>'+item[0]+'</span><b><strong>'+item[1].toLocaleString()+'</strong><small> asset'+(item[1]===1?'':'s')+'</small></b></div>').join('');
    ...
    if(conditionChartFilter){conditionChartFilter.innerHTML=optionList(assetsData.conditions,'All conditions');conditionChartFilter.value=condition.value;conditionChartFilter.addEventListener('change',()=>{condition.value=conditionChartFilter.value;updateKpis()})}
  ...
  window.renderFixedAssetsKpis=(metrics=source,scopeLabel=metrics.scopeLabel||'Selected asset records')=>{
    ...
    assetTabCard.innerHTML='<nav class="budget-portfolio-tabs manpower-planning-tabs fixed-asset-tabs" role="tablist" aria-label="Fixed asset views"><button type="button" class="manpower-planning-tab active" data-fixed-asset-tab="company" role="tab" aria-selected="true"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 21h16M6 21V7l6-4v18M12 10h6v11M9 9h1M9 13h1M9 17h1M15 14h1M15 18h1"/></svg>Company Asset Inventory</button><button type="button" class="manpower-planning-tab" data-fixed-asset-tab="overview" role="tab" aria-selected="false"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="3" width="16" height="18" rx="2"/><path d="M8 8h8M8 12h8M8 16h5"/></svg>Fixed Asset Overview</button><button type="button" class="manpower-planning-tab" data-fixed-asset-tab="devices" role="tab" aria-selected="false"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="12" rx="2"/><path d="M8 20h8M12 16v4"/></svg>Laptop &amp; Desktop</button></nav>';
    panel.append(assetTabCard);
    const fixedAssetTabs=assetTabCard.querySelector('.budget-portfolio-tabs');fixedAssetTabs?.classList.remove('budget-portfolio-tabs');fixedAssetTabs?.classList.replace('manpower-planning-tabs','unified-tabs');fixedAssetTabs?.querySelectorAll('.manpower-planning-tab').forEach(tab=>tab.classList.replace('manpower-planning-tab','unified-tab'));
    ...
    const overviewTab=assetTabs.find(tab=>tab.dataset.fixedAssetTab==='overview');if(overviewTab?.querySelector('svg'))overviewTab.querySelector('svg').innerHTML='<path d="m3 7 9-4 9 4-9 4-9-4Z"/><path d="M3 7v10l9 4 9-4V7M12 11v10"/><path d="m7 9 9 4"/>';
    ...
    const setOverviewIcon=()=>{const icon=assetTabCard.querySelector('.fixed-asset-overview-table-card .unified-table-title-icon');if(icon)icon.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m3 7 9-4 9 4-9 4-9-4Z"/><path d="M3 7v10l9 4 9-4V7M12 11v10"/><path d="m7 9 9 4"/></svg>'};
    assetTabs.find(tab=>tab.dataset.fixedAssetTab==='overview')?.addEventListener('click',()=>requestAnimationFrame(()=>{assetTabCard.querySelectorAll('.fixed-asset-overview-table-card,.fixed-asset-latop-table-card').forEach(card=>card.remove());renderOverviewRecord();setupOverviewSearch();setOverviewIcon();enhanceOverviewConditions();enhanceFixedAssetCodes();setupOverviewPagination()}));
    const renderLaptopDesktopOverview=()=>{assetTabCard.querySelectorAll('.fixed-asset-overview-table-card,.fixed-asset-latop-table-card').forEach(card=>card.remove());assetRecords=assetRecords.filter(record=>/^(Laptop|Desktop)$/i.test(String(record.t||'')));const overview=assetTabs.find(tab=>tab.dataset.fixedAssetTab==='overview');overview?.classList.add('active');renderOverviewRecord();overview?.classList.remove('active');setupOverviewSearch();setOverviewIcon();enhanceOverviewConditions();enhanceFixedAssetCodes();setupOverviewPagination();assetTabCard.querySelector('.fixed-asset-overview-table-card')?.classList.replace('fixed-asset-overview-table-card','fixed-asset-latop-table-card')};
    assetTabs.find(tab=>tab.dataset.fixedAssetTab==='devices')?.addEventListener('click',()=>requestAnimationFrame(renderLaptopDesktopOverview));
    assetTabs.filter(tab=>tab.dataset.fixedAssetTab!=='overview').forEach(tab=>tab.addEventListener('click',()=>assetTabCard.querySelectorAll('.fixed-asset-overview-table-card,.fixed-asset-latop-table-card').forEach(card=>card.remove())));
    ...
    ['fixedAssetPeriod','fixedAssetCompany','fixedAssetDepartment','fixedAssetType','fixedAssetBrand','fixedAssetCondition','fixedAssetMonth','fixedAssetYear','fixedAssetFrom','fixedAssetTo'].forEach(id=>panel.querySelector('#'+id)?.addEventListener('change',()=>requestAnimationFrame(()=>{if(assetTabs.find(tab=>tab.dataset.fixedAssetTab==='devices')?.classList.contains('active'))renderLaptopDesktopOverview()})));
